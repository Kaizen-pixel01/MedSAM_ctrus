...
#specifying where the ground truth (gt) masks are 
...
for filename in tqdm(os.listdir(mask_dir)): #looping thru all masks
    ...
    if mask is None:
        print(f"Skipped unreadable: {filename}") #in case there were issues when downloading the images
        continue
    ...
    if not (binary_mask > 0).any():
        print(f"No foreground in: {filename}") #to avoid image pairs with no masks
        continue
